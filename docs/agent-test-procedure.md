...
5. **Subagent lifecycle.** Faux-driven main agent calls `spawn_agent` twice → both run concurrently (overlapping event timestamps); `steer_agent` aborts the target subagent's current step and restarts it with the `[INTERRUPTED]` note, WITHOUT triggering a premature `<subagent_result>` (the `interrupting` flag guard); completion interrupts the main agent with a `<subagent_result>` turn (both while it is idle and while it is streaming, in which case the main prompt carries the `[INTERRUPTED]` note); a rejected subagent run delivers a `failed` result rather than leaking; `steer_agent`/`check_agent` on unknown or finished ids return tool errors, not crashes; a subagent `ask_user` question interrupts the main agent and `answer_subagent` resolves it round-trip. Run the steer-during-run case in a loop (50+ iterations): the flag guard is protecting against a settle-order race that a single happy-path run will usually pass.
6. **Main-session delivery lock.** The nastiest race in the design: two subagents complete in the same tick, and a subagent completes at the exact moment a voice `runTask` lands. Assert every message reaches the main session (session JSONL contains all `<subagent_result>`s and the task; no unhandled rejection from pi's "already processing" error). Without the `injectMain` single-flight lock this drops messages silently, so this scenario is the lock's regression test.
...
8. **Voice dispatcher ops.** Every voice tool (`send_task_to_agent`, `answer_agent`, `get_agent_status`, `look_at_board`, `abort_agent`) round-trips against the mocked bridge; in particular `look_at_board` must hit the `get-scene-summary` op and return counts/texts, not undefined.
