...
5. **Subagent lifecycle.** Faux-driven main agent calls `spawn_agent` twice → both run concurrently (overlapping event timestamps); `steer_agent` lands in the target subagent's session; `wait_agent` returns the scripted final text; `wait_agent` on unknown id returns a tool error, not a crash.
