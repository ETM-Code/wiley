...
describe("diagram layout quality", () => {
  ...
  it("draws a star's centre with weight and leaves every spoke whole", async () => {
    ...
    // And every spoke's centre stands on one circle. Equalising the connectors
    // instead leaves the centres at whatever radius each box's own size puts
    // them, and a ring of centres that is not a circle is the thing a reader
    // sees, whatever the connectors underneath it are doing.
    ...
    const radii = Array.from({ length: spokes }, (_, index) => {
      const at = centre(box(`s${index}`));
      return Math.hypot(at.x - middle.x, at.y - middle.y);
    });
    // Within a cell's half-diagonal, which is as near a circle as points
    // seated on a grid can stand. Equal connectors put this at 172 on the
    // same shape.
    expect(Math.max(...radii) - Math.min(...radii))
      .toBeLessThanOrEqual(MODEL_GRID_SIZE * Math.SQRT1_2);
  });
