...
function canvasTools(host: PiToolHost, agentId: string): ToolDefinition[] {
  return [
    ...
    defineTool({
      ...
      description: "Evolve a diagram you already drew instead of drawing it again. Name it with diagram: its diagramId, or the id of any element inside it; the canvas context lists every diagram on the board. Never redraw a diagram you own, and never clear the canvas to change one; this keeps the surviving shapes as the same elements, animates them to their new places, and leaves the user's own drawings alone. mode merge (the default) overlays what you pass onto the graph already on the board: a node or container you name by id is replaced, an edge you name by its endpoints is replaced, and everything you leave out survives. A node you add here takes the same shape vocabulary draw_diagram accepts: rectangle, diamond, ellipse, or text, and nothing else. mode replace treats what you pass as the entire graph, which is how you delete. The diagram is rebuilt from the board, so the theme, labels, shapes, and containers carry over, but the layout algorithm and direction do not: pass layout again if you want something other than the default. keepPosition (default true) pins the result to the corner the diagram already occupies and reports shifted if growth forced it aside. An edge may reach into the user's own drawing: name one of their shapes as an endpoint with its element id from humanGraph in the canvas context (human:ELEMENT_ID, or the bare element id). Their shape is never redrawn, moved, or copied; the arrow binds to the real element and routes around everything they drew, so extending a hand-drawn sketch means connecting to it rather than drawing your own version beside it. The result returns counts of what was added, removed, moved, and relabeled.",
      parameters: Type.Object({
        diagram: Type.String(),
    ...
    defineTool({
      name: "edit_canvas",
      label: "Edit Canvas",
      description: "Patch or delete existing elements by id, including the user's hand-drawn ones: move, resize, recolor, restyle, or change text. Setting text or fontSize on a labelled shape automatically edits its attached label, moving a shape carries its label and bound arrows along, and deleting a shape removes its label. Read the canvas first and change only necessary properties. Touching one of the user's own elements is for the change they actually asked for on that element; never move or delete their work to make room for yours, and never rearrange their sketch this way (tidy_diagram does that, on request).",
