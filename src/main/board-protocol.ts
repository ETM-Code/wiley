/**
 * The canonical board protocol.
 *
 * Wiley's agent prompts and the live-excalidraw Pi skill must say the same
 * thing about the board, so the prose lives here once and both consumers
 * compose it. The Pi SDK injects only a skill's name, description, and path
 * into context (never its body), so the agent prompt has to carry the rules
 * inline; scripts/sync-skill.ts writes the same text into the skill file
 * between its board-protocol markers, and tests/board-protocol.test.ts fails
...
export const VISIBLE_PROCESS_RULES = `
...
export const HUMAN_ELEMENT_RULES = `
